...
def rename_step_bodies(in_path, out_path):
    ...
    as the body name shown in the model tree.

    gmsh always writes one extra PRODUCT entry as an assembly wrapper
    (suffix "1", no dots) before the actual body entries. We skip that
    wrapper and rename actual bodies only:
        First actual body  → "Epoxy_matrix"   (the cube with holes)
        Remaining bodies   → "AlN_particle_001", "AlN_particle_002", ...
    ...
    Returns
    -------
    n_bodies : int — total number of actual bodies renamed
    """
    ...
    for i, m in enumerate(matches):
        # The assembly wrapper has suffix "1" (0 dots) — skip it
        suffix    = m.group(2).split(' ')[-1]
        dot_count = suffix.count('.')

        if dot_count == 0:
            # Assembly wrapper — do not rename
            continue
        elif i == 1:
            # First actual body after the wrapper = Epoxy matrix
            new_name = "Epoxy_matrix"
        else:
            # All remaining bodies = AlN spheres
            sphere_count += 1
            new_name = f"AlN_particle_{sphere_count:03d}"
